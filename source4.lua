...
AimbotFOVCircle.Color = AimbotFunctions.FOVColor
AimbotFOVCircle.Thickness = 1
AimbotFOVCircle.NumSides = 50
AimbotFOVCircle.Radius = AimbotFunctions.FOV
    ...
    if AimbotFunctions.Enabled then
      ...
      if Character and Character:FindFirstChild("Head") then
            ...
            if OnScreen then
              ...
              if MouseDistance <= AimbotFunctions.FOV and WorldDistance <= AimbotFunctions.Distance then
                ...
                if CanSee then
                  ...
                  table.insert(CurrentTargets, {
                    Part = PartToAim, 
                    MouseDist = MouseDistance, 
                    WorldDist = WorldDistance, 
                    Health = Health,
                    MaxHealth = MaxHealth,
                    Player = Player,
        ...
        if #CurrentTargets > 0 then
          local PriorityType = AimbotFunctions.TargetPriority
          if type(PriorityType) == "table" then
            PriorityType = PriorityType[1] or "Closest to Crosshair"
          end
          ...
          if PriorityType == "Closest to Crosshair" then
            ...
            for _, v in pairs(CurrentTargets) do
              local DistanceFromCrosshair = (v.ScreenPos - CrosshairCenter).Magnitude
              local CurrentDistance = (SelectedTarget.ScreenPos - CrosshairCenter).Magnitude
...
RunService.RenderStepped:Connect(function()
    if AimbotFunctions.DrawFOV and AimbotFunctions.Enabled then
        local Center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
...
Aimbot:AddSlider(".", {
  ...
  Visible = true,
  Callback = function(Value)
    AimbotFunctions.Smoothness = Value
  end,
})
